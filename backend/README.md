...
1. Using File Manager or FTP, upload the entire `/backend` folder to your hosting account
...
composer install
# Then upload the entire backend folder including vendor/
```
...
Hostinger typically auto-detects `.htaccess`. Ensure it's in the backend folder:
...
- If you can't change php.ini, add `.user.ini` in backend folder:
  ```
